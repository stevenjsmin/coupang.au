...
public class AccessCheckFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		if (session != null) {
			sessionUser = (SessionUserInfo) session.getAttribute(CoupangConstants.LOGIN_USER_SESSION_ATTR);
		}
		...
		if (ignorUrlList.contains(url) || StringUtils.startsWith(url, "/guest/") || StringUtils.startsWith(url, "/order/") || StringUtils.startsWith(url, "/shop/")) {
			...
			if (StringUtils.equals(url, "/common/auth/restrictArea.pang")) {
				if (sessionUser != null) {
			...
			if (session == null) {
				...
				if (sessionUser != null) {
					if (accChecker.hasAuth(request) || StringUtils.endsWithIgnoreCase(sessionUser.getUser().getUserId(), "admin")) {
						...
						response.sendRedirect("/common/auth/restrictArea.pang");
					}
				} else if (sessionUser == null && StringUtils.equals(url, "/common/auth/sessionExpired.pang")) {
					chain.doFilter(req, res);
				} else {
					 response.sendRedirect("/common/auth/sessionExpired.pang");
