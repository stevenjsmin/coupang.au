 ...
 *
 */
public class CoupangDataSource extends BasicDataSource {

	private static final Logger logger = LoggerFactory.getLogger(CoupangDataSource.class);
	...
	public CoupangDataSource() {
		...
		try {
			...
			File f = FileUtils.convertInputStreamToFile(CoupangDataSource.class.getResourceAsStream("/configuration/jdbc.properties"));
			...
			String db_version = coupang.getString("DB.VER");

			if (StringUtils.isBlank(env)) env = "local";
			if (StringUtils.isBlank(client)) client = "coupang";
