...
package coupang.framework.auth;
...
import coupang.framework.CoupangConstants;
...
public class AuthController extends BaseController {
	...
	public ModelAndView welcome(HttpServletRequest request) throws Exception {
		...
		if (authService.alreadyLogin(request)) {
			...
			SessionUserInfo sessionUser = (SessionUserInfo) session.getAttribute(CoupangConstants.LOGIN_USER_SESSION_ATTR);

			Role role = sessionUser.getSessionRole();

			if (StringUtils.equalsIgnoreCase(role.getRoleId(), CoupangConstants.USER_TYPE_ADMIN)) {
				viewName = "tiles/admin/welcome";
			} else if (StringUtils.equalsIgnoreCase(role.getRoleId(), CoupangConstants.USER_TYPE_SELLER)) {
	...
	public void logout(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		response.sendRedirect("/common/auth/welcome.pang");
	...
	public Map<String, Object> login(HttpServletRequest request) throws Exception {
		...
		if (StringUtils.contains(userId, "/")) {
			...
			sessionRole = CoupangConstants.USER_TYPE_CUSTOMER;
		}

		String message = "";
		User user = userService.getUserInfo(userId);
		boolean valid = userService.validateUser(userId, password);
		String status = CoupangConstants.LOGIN_AUTH_STATUS_FAILURE;
			...
			if (StringUtils.equalsIgnoreCase(user.getUseYn(), "Y")) {
				...
				if (valid) {
					...
					if (StringUtils.equalsIgnoreCase(user.getApplyStatus(), "COMPLETE")) {
						status = CoupangConstants.LOGIN_AUTH_STATUS_SUCCESS;
						...
						if (StringUtils.isNotBlank(sessionRole)) {
							...
							if (hasRole) {
								...
								Role role = roleService.getRoleInfo(CoupangConstants.USER_TYPE_CUSTOMER);
								authService.getSessionUserInfo(request).setSessionRole(role);
							}
						}

						// 비밀번호 변경 기간이 되었는지 체크
						if (isPasswordChangePeriod) status = CoupangConstants.LOGIN_AUTH_STATUS_CHANGE_PWD;

					} else {
						status = CoupangConstants.LOGIN_AUTH_STATUS_FAILURE;
