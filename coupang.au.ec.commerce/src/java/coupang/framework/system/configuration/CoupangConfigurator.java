...
package coupang.framework.system.configuration;
...
import coupang.framework.uitl.FileUtils;
 ...
 *
 */
public final class CoupangConfigurator {
	// public static XMLConfiguration SYS_CONFIG = null;
	public static CoupangConfig SYS_CONFIG = null;

	private static final Logger logger = LoggerFactory.getLogger(CoupangConfigurator.class);

	private static CoupangConfigurator singleton;

	private CoupangConfigurator() throws Exception {
		
		Configuration coupang = new PropertiesConfiguration(System.getProperty("COUPANG_CONFIG"));
		String env = coupang.getString("SYS.ENV_TYPE");
		
		String client = coupang.getString("SYS.CLIENT");
		...
		try {
			...
			if (StringUtils.isBlank(env)) {
				...
				if (!StringUtils.equalsIgnoreCase(env, "LOCAL") && !StringUtils.equalsIgnoreCase(env, "DEV") && !StringUtils.equalsIgnoreCase(env, "TEST")
					...
					throw new Exception("Coupang eCommerce Engin can't decide what kind of server type. Please make sure server type by indicate ENV_TYPE as JVM option");
				} else {
		...
		final File commonConfigURL = FileUtils.convertInputStreamToFile(CoupangConfigurator.class.getResourceAsStream(commonConfigurationFile));
		final File clientConfigURL = FileUtils.convertInputStreamToFile(CoupangConfigurator.class.getResourceAsStream(clientConfigurationFile));

		try {
			CoupangConfig commonConfig = new CoupangConfig(commonConfigURL);
			CoupangConfig clientConfig = new CoupangConfig(clientConfigURL);
			...
			SYS_CONFIG = new CoupangConfig(cc);
			...
			try {
				ApplicationProperties.appInit(coupang);
			} catch (Exception e) {
	...
	public static CoupangConfigurator getInstance() throws Exception {
		if (singleton == null) {
			singleton = new CoupangConfigurator();
		}
		return singleton;
	}
