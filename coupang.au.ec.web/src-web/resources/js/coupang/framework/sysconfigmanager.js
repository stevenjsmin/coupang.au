...
function goDetailInfo(stage, key){
	document.location.href = "/framework/sysconfigmanager/detailSystemConfigForm.pang?stage=" + stage + "&key=" + key;
}
    
function goList() {
	document.location.href = "/framework/sysconfigmanager/Main.pang";
} 

function goModify(){
	document.location.href = "/framework/sysconfigmanager/modifySystemConfigForm.pang?stage=" + _STAGE + "&key=" + _KEY;
} 

function add(){
	document.location.href = "/framework/sysconfigmanager/registSystemConfigForm.pang";
}
...
function save(){
      ...
      $.ajax({
           url  : "/framework/sysconfigmanager/saveSystemConfig.pang",
           data      : {
             stage : stages,
             key : key,
             value : value,
...
function deleteInfo(stage,key){
	...
	BootstrapDialog.confirm({
        ...
        callback: function(result) {
            if(result) {
			      $.ajax({
			           url  : "/framework/sysconfigmanager/systemConfigDelete.pang",
			           data      : {
			             stage : stage,
			             key : key
			           },
