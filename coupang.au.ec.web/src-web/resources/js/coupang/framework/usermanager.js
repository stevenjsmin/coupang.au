...
function goDetailInfo(userId){
	document.location.href = "/framework/usermanager/detailUserForm.pang?userId=" + userId;
}
    
function goList() {
	document.location.href = "/framework/usermanager/Main.pang";
} 

function goModify(){
	document.location.href = "/framework/usermanager/modifyUserForm.pang?userId=" + _USERID;
} 

function add(){
	document.location.href = "/framework/usermanager/registUserForm.pang";
}
...
function save(){
      ...
      $.ajax({
           url  : "/framework/usermanager/saveUser.pang",
           data      : {
             userId : userId,
             userName : userName,
...
function setInitialize(userId){
	$.ajax({ url : "/framework/usermanager/initialUserAccount.pang?userId=" + userId,
...
function setSuburbCbx(postcodeObjId, objName){
	...
	$.ajax({ url : "/common/postcode/suburbCmbx.pang?postcode=" + postcode + "&objName=" + objName,
...
function deleteInfo(){
	...
	BootstrapDialog.confirm({
        ...
        callback: function(result) {
            if(result) {
			      $.ajax({
			           url  : "/framework/usermanager/deleteUser.pang",
			           data      : {
			             userId : _USERID
			           },
...
function deleteProfilePhoto (userId) {
    $.ajax({
         url  : "/framework/usermanager/deleteProfileImage.pang",
         data      : {
           userId : userId
         },
...
function openPopupForRegisterProfilePhoto(userId) {
    $("#userProfilePhotoPopup").kendoWindow({
        content: "/framework/usermanager/profileImageUploadForm.pang?userId=" + userId,
